        ...
        _tasks += `                                         
            <div class="task-item ${task.state === "show" ? 'mt-2 d-flex justify-content-between align-items-center' : 'd-none'}">
                ...
                <div class="action btns">
                    <button type="button" class="btn btn-sm btn-success is__completed" onclick="todoCompleted('${task.id}')"><i class="fa-solid fa-circle-check"></i></button>
                    <button type="button" class="btn btn-sm btn-primary edit" onclick="updateTask('${task.id}')"><i class="fa-solid fa-pen-to-square"></i></button>
                    <button type="button" class="btn btn-sm btn-danger ms-1 delete" onclick="deleteTask('${task.id}')"><i class="fa-solid fa-trash-can"></i></button>
                </div>
            </div>
        `;
    });
                    ...
                    input.addEventListener('keydown', e => {
                        if (e.key === 'Enter') {
                            ...
                            saveEditTodo.addEventListener('click', e => {
                                ...
                                if (input_value) updateTask(task, input_value);
                            });
...
search_btn.addEventListener('click', e => {
    ...
    if (search_value != '') {
        ...
        input.value = '';
        searchTask(search_value.toLowerCase());
    } else {
